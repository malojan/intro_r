...
head(elec, 2)
tail(elec, 2)
dplyr::glimpse(elec) # from the dyplr package
colnames(elec)
dim(elec)
nrow(elec)
summary(elec)
...
One thing you need to know is thay there are many ways to do the same thing in R and you do not have to memorize them. You need to understand the logic, to learn how to learn. 

## Data import and export

### Get data

So far, we created some data from scratch for the sake of the example. But from now, we will be interested in working with real data to analyze it. The first stage in every data analysis project is to get/collect/produce data. Then the first thing you need is to get that data somewhere, either collect if through a survey, or build a dataset yourself by coding some informations about the individuals or organizations you are interested in. But there are also many many many datasets already available around there that people (administrations, researchers, ngos, companies, international organization and so on) have already created and made available. In Appendix of the course, you will find a lists of them that you can explore if you want. 
...
At this stage, I will give you already-made datasets on which you can work, but you should know that with R, you can collect data directly from the internet : this is called web scraping (you can learn that later on if you continue to)
...
-   `read_r` package : `read_csv()`, `read_csv2()`
...
I already told you about replication and eveyrhing. Rstudio allows you to combine text and code in the same documents. I will try to -   Or literate programming documents : Quarto/Rmarkdown : documents that allows you to combine text and code and then to convert them in pdf, word or html format.
introduce you to these tools because there are really nice to write.
