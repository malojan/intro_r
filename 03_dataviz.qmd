...
In this session, you will learn how to create graphs. Data visualization serves two primary purposes: data exploration and communication. When we examine our data, a few graphs can provide valuable insights into its distribution and the characteristics of various variables. Additionally, when we analyze data and wish to convey our findings, graphs can be highly effective in conveying key information. This notion is captured by the phrase "A picture is worth a thousand words." Furthermore, aesthetics play a significant role, and we appreciate visually pleasing graphs. R is capable of producing exceptionally beautiful graphs, and once you learn how to create them, you may develop a strong dislike for those who rely on Excel graphs for the rest of your life.

There are several methods available for creating graphs in R. Base R provides functions such as `plot()`, `hist()` or `boxplot()` (you can also explore the `lattice` package). However, in this session, we will focus on a widely popular package called ggplot. It is a component of the tidyverse, so there is no need to install or load it separately if the tidyverse package is already loaded.

The "gg" in `ggplot` represents the "grammar of graphics." Graphs are constructed by adding various layers to a basic graph, allowing for progressively more complex modifications such as adjusting the title or adding annotations.
...
1. Forget about pie charts. 

One of the first graph that comes in our mind when we think about data vizualisation are pie chart (or camemberts as we say in french). If you have already done that once in your life, I warn you that it will not happen again. 

- Here details on what makes a good or bad vizualisation. 


If you want to learn more about graphs, I recommand you to read this and this. Healy, Winston Chang


https://medium.com/@fneves/stop-using-those-ugly-pie-charts-962ac70beb41
...
### Exercice 3 : vizualising political parties

1. Download and import the [Chapell Hill Expert Survey trend file](https://www.chesdata.eu/ches-europe) into Rstudio

2. Explore the codebook to see the variables available in this dataset and choose one of your interest^[Variables on international security or position towards the US might be of interest for IR students for instance]. 
...
- Kieran Healy's has written a whole [book](https://socviz.co/index.html#preface) on dataviz. 
